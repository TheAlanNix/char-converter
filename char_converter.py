...
def replace_0x92_in_file(filepath):
    with open(filepath, 'rb') as f:
        content = f.read()
    count = content.count(b'\x92')
    if count > 0:
...
def process_file(filepath, write=False, debug=False):
    if debug:
        print(f"Scanning: {filepath}")
    count = count_0x92_in_file(filepath)
    if count > 0:
        if write:
            replace_0x92_in_file(filepath)
        else:
            print(f"Found {count} occurrence(s) in: {filepath}")


def find_and_convert(path, write=False, debug=False):
    if os.path.isfile(path):
        process_file(path, write, debug)
    else:
        for dirpath, _, filenames in os.walk(path):
            for filename in filenames:
                filepath = os.path.join(dirpath, filename)
                process_file(filepath, write, debug)
...
def main():
    ...
    find_and_convert(args.path, args.write, args.debug)
